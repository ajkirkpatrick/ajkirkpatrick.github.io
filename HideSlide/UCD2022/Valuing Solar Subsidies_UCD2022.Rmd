...
subtitle: "UC Davis - April 2022"
author: "Bryan Bollinger (NYU), Ken Gillingham (Yale)<br>Justin Kirkpatrick (Michigan State), Steve Sexton (Duke)<br><br>Slides at: .orange[bit.ly/UCD2022]"
date: "<br><br><br><br>.font50[Last updated `r format(Sys.Date(), '%B %d, %Y')`]"
output:
  xaringan::moon_reader:
    lib_dir: libs
    yolo: false
    css: [default, metropolis, metropolis-fonts, ajk_css-Apr2022]
    nature: 
...
# 
knitr::include_graphics(file.path(imageFolder, 'rates_avg_v2.png'))

```

$\theta$ converts $q^{*}\bar{p}$, the per-period flow payoff, into a present value.

$\delta=15.2%$ implies $\theta=39.54$, or 2.41x the NPV at a government discount rate of 4%.
...
# see "Explore_Results_v2.Rmd"
# 
knitr::include_graphics(file.path(imageFolder, 'values_avg_v2.png'))

```

The average present value of average flow $q^{*}\bar{p}$ is $8,595. The same flow discounted at a government discount rate of 4% is $20,702.
...
# see "Explore_Results_v2.Rmd"
# 
knitr::include_graphics(file.path(imageFolder, 'rates_wealth_v2.png'))
...
### Counterfactuals
- Structural parameters permit estimation under any flow and up-front subsidies
- NEM 3.0 components
  - Two-channel billing: Expected net export (by time)
  - Per-kW capacity subsidies: Per $K^{*}$ subsidies over time (additional flow)
